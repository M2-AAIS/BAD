...
contains
  ...
  subroutine curve()  
    ...
    real(kind = x_precision), parameter                    :: t_min = 3.d-1
    real(kind = x_precision), parameter                    :: t_max = 2.49d0
    !real(kind = x_precision), parameter                    :: t_min = 2.d0
    ...
    do j              = 1 , 1
       ...
       fname = 'Temperature_Sigma_'//trim(number_of_cell)//'_2.dat'
       ...
       !  write(fid,*)"T     &         Sigma"
        ...
        !  call display_variables(temp,Omega,r, sigma, H, rho, cs, nu, Q_plus, Q_minus,&
         !      K_ff, K_e, tau_eff, P_rad, P_gaz,E_ff,Fz,f)
          ...
          temp_reel   = log10(temp * T_0)
          sigma_reel  = log10(sigma * sigma_0)
