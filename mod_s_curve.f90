...
contains
  ...
  subroutine curve()
    ...
    real(kind = x_precision), parameter                    :: t_min = 1.d-4
    real(kind = x_precision), parameter                    :: t_max = 1.d1
    ...
    do j              = 1 , 1
       ...
       !  write(fid,*)"T     &         Sigma"
        do i          = 1, nb_it
          ...
          write(fid,'(1p,E12.6,4x,1p,E12.6,4x,1p,E12.6)')sigma_real,temp_real, tau_eff
       enddo
       close(fid)
    enddo
  ...
  subroutine quadratic(coeff_a, coeff_b, coeff_c, sol)
    ...
    if (delta < 0.) then
      ...
      if (sol_1 < 0.d-12) then
         write(*,*)'There is a problem with the solution of the quadratic equation.'
         stop
      endif
  ...
  subroutine initial_variables(rs, rmin, Mdot_0, Sigma_0, Omega_0, T_0, rho_0)
    ...
    !T_0                = 1._x_precision / 3_x_precision * ((Mdot_0 * c**2) / &
    !     (48._x_precision * pi * rs**2 * stefan))**(0.25_x_precision)
  ...
  subroutine variables(T, Sigma, Omega, H, rho, cs, nu, Q_plus, Q_minus,&
    ...
    real(kind = x_precision)                                 :: nu_0
    real(kind = x_precision)                                 :: Fz_0
    real(kind = x_precision)                                 :: K_ff_0
    real(kind = x_precision)                                 :: Q_plus_0
    real(kind = x_precision)                                 :: Q_minus_0
    ...
    nu_0                 = 2._x_precision * rs**2 * Omega_0/3._x_precision
    Fz_0                 = Sigma_0 * Omega_0 / 4._x_precision
   ...
   ! end if
    ...
    Q_plus_0 = Omega_0**3 * rs**2

    Q_minus_0   = 2._x_precision * Fz_0 / sigma_0
  ...
  real(kind=x_precision) function dichotomy(Smin, Smax, eps, T, omega, sigma_0, Omega_0,rs, T_0, rho_0)
    ...
    real(kind=x_precision),intent(in)                        :: rho_0
    real(kind=x_precision),intent(in)                        :: rs
    real(kind=x_precision)                                   :: Scenter = 0._x_precision
    ...
    call variables(T, Scenter, Omega, H, rho, cs, nu, Q_plus, Q_minus, K_ff,&
         K_e, tau_eff, P_rad, P_gaz,E_ff,Fz,f_center,Sigma_0, Omega_0,rs,T_0, rho_0)
          ...
          Scenter       = (Smin + Smax) / 2._x_precision
